...
import { Menu, X, Rocket, ExternalLink, User, LogOut, ChevronDown, LayoutDashboard } from 'lucide-react';
...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem('vexon_user');
    if (storedUser) {
  ...
  const handleLogin = () => {
    ...
    const redirectUri = encodeURIComponent('http://localhost:3000/callback'); // Production: vexonapi.teymensel.com relative
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('vexon_user');
    setUser(null);
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled || isDashboard ? 'bg-[#0f1016]/90 backdrop-blur-md border-b border-white/5' : 'bg-transparent'
        }`}
    >
            ...
            <div className="flex items-center gap-4">
              {user ? (
                <div className="relative">
                  ...
                  >
                    <img
                      src={user.avatar ? `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png` : `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.id}`}
                      alt="User"
                  ...
                  {userDropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-56 bg-[#181920] border border-white/10 rounded-xl shadow-2xl py-2 animate-in fade-in zoom-in-95 duration-200">
                      <div className="px-4 py-3 border-b border-white/5 mb-2">
                        ...
                        <p className="text-xs text-gray-400">@{user.discriminator !== '0' ? user.discriminator : 'discord'}</p>
                      </div>

                      <Link to="/dashboard" onClick={() => setUserDropdownOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-300 hover:text-white hover:bg-white/5 transition-colors">
                        <LayoutDashboard size={16} />
                        Sunucularım
                      </Link>
                      <Link to="/premium" onClick={() => setUserDropdownOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-300 hover:text-white hover:bg-white/5 transition-colors">
                        <User size={16} />
                        Abonelikler
                      </Link>
