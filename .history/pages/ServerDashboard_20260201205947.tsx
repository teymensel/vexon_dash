...
const mockModules: DashboardModule[] = [
  // Sunucu Yönetimi (Temel)
  {
    id: 'automation',
    title: 'Otomasyon',
    description: 'Mesajlar, tepkiler ve rol değişiklikleri gibi sunucu olaylarına yanıt olarak bot eylemlerini otomatikleştirin.',
    icon: Workflow,
    category: 'Management',
    isPremium: true
  },
  {
    id: 'welcome',
    title: 'Hoşgeldin & Hoşçakal',
    description: 'Yeni üyelerinize otomatik bir karşılama mesajı gönderin ve onlara roller verin.',
    icon: Hand,
    category: 'Management',
    isPremium: true,
    isEnabled: true
  ...
  {
    id: 'welcome-channel',
    title: 'Karşılama Kanalı',
    description: 'Yeni üyeleri karşılamak ve gerekli bilgileri paylaşmak için özel bir yer.',
    icon: MessageSquare,
    category: 'Management',
    isPremium: true
  },
  {
    id: 'custom-commands',
    title: 'Özel Komutlar',
    description: 'Kendi metin komutlarınızı veya belirli roller veren komutlarınızı oluşturun',
    icon: MessageCircle,
    category: 'Management',
    isPremium: true
  },
  {
    id: 'reaction-roles',
    title: 'Tepki Rol',
    description: 'Bir mesaja tepki vererek üyelerinizin rol almasına izin verin',
    icon: Smile,
    category: 'Management',
    isPremium: true
  },
  {
    id: 'invite-tracking',
    title: 'Davet Takibi',
    description: 'Üyelerinizin topluluğunuza kaç kişi getirdiğini takip edin ve onları bir sıralama sayfasında görüntüleyin',
    icon: UserPlus,
    category: 'Management',
    isNew: true
  },
  {
    id: 'tickets',
    title: 'Talep',
    description: 'Üyelerinizin destek, şikayet ve öneri için talep oluşturmasına izin verin.',
    icon: Ticket,
    category: 'Management',
    isPremium: true
  },
  ...
  // Araçlar
  {
    id: 'starboard',
    title: 'Starboard',
    description: 'En sevilen mesajları yıldızlayarak öne çıkarın.',
    icon: Star,
    category: 'Tools',
    isNew: true
  },
  {
    id: 'emoji',
    title: 'Emoji',
    description: 'Sunucunuzu özel emojiler ekleyerek geliştirin',
    icon: Smile,
    category: 'Tools',
    isEnabled: true
  },
  {
    id: 'polls',
    title: 'Anket',
    description: 'Üyelerinizin anket oluşturmasına ve oylamasına izin verin',
    icon: BarChart,
    category: 'Tools'
  },
  {
    id: 'embeds',
    title: 'Gömülü mesaj',
    description: 'Küçük resimler ve renklerle kurallarınız ile duyurularınız için güzel gömülü mesajları oluşturun',
    icon: Layout,
    category: 'Tools'
  },
  {
    id: 'search',
    title: 'Bir Şey Ara',
    description: 'Youtube videolarını, twitch yayıncılarını, animeleri ve daha fazlasını aramak için komutları etkinleştirin',
    icon: Search,
    category: 'Tools'
  },
  {
    id: 'help',
    title: 'Yardım',
    description: 'Sunucunuz için kontrol panelini ve yardım komutlarını etkinleştirir',
    icon: HelpCircle,
    category: 'Tools',
    isEnabled: true
  },
  {
    id: 'reminder',
    title: 'Hatırlatıcı',
    description: 'Birkaç dakikada bir veya saatte bir tekrarlanan özel mesajlar gönderin',
    icon: Clock,
    category: 'Tools'
  },
  {
    id: 'stats-channels',
    title: 'İstatistik Kanalları',
    description: 'Sunucu istatistiklerinizi ve sosyal medya takipçi sayınızı sunucu kanallarınızın kenar çubuğunda gösterin',
    icon: BarChart2,
    category: 'Tools',
    isPremium: true
  },
  {
    ...
    description: 'Üyelerinizin tek bir tıklamayla sunucunuzda geçici sesli kanallar oluşturmasına izin verin',
    icon: Volume2,
    category: 'Tools',
    isPremium: true
  },
  ...
  { id: 'levels', title: 'Seviye', description: 'Üyelerinize mesaj gönderdiklerinde TP ve Seviye verin ve onları aktifliklerine göre sıralama sayfasında gösterin', icon: Trophy, category: 'Fun', isPremium: true },
...
// Mock server data needed for sidebar
...
const ServerDashboard: React.FC = () => {
  ...
  // In a real app, you would fetch server details using guildId
  const server = mockServer; 

  const managementModules = mockModules.filter(m => m.category === 'Management');
  const toolsModules = mockModules.filter(m => m.category === 'Tools');
  const socialModules = mockModules.filter(m => m.category === 'Social');
  const funModules = mockModules.filter(m => m.category === 'Fun');
  const monetizationModules = mockModules.filter(m => m.category === 'Monetization');
  const web3Modules = mockModules.filter(m => m.category === 'Web3');
  const securityModules = mockModules.filter(m => m.category === 'Security');
    ...
    <div className="flex pt-20 bg-[#0f1016] min-h-screen">
      ...
      <main className="flex-grow p-6 md:p-12 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Ad Banner */}
          <div className="relative w-full h-32 md:h-40 bg-gradient-to-r from-[#181920] to-[#0f1016] rounded-xl border border-white/5 mb-12 flex items-center justify-between px-8 overflow-hidden group">
             ...
             <div className="relative z-10">
                ...
                <h3 className="text-xl md:text-2xl font-bold text-white">BU ALANA REKLAM VERİN</h3>
                <p className="text-gray-500 text-xs md:text-sm mt-1">Yüzlerce yeni üyeye ulaşın</p>
             </div>
             <div className="relative z-10 hidden md:block">
                 <button className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-white text-xs font-bold transition-colors">
                    REKLAM
                 </button>
             </div>
          </div>
          ...
          {[{ title: 'Sunucu Yönetimi', modules: managementModules, id: 'server-management' },
            { title: 'Araçlar', modules: toolsModules, id: 'tools' },
            { title: 'Sosyal Medya Bildirimleri', modules: socialModules, id: 'social' },
            { title: 'Oyun & Eğlence', modules: funModules, id: 'fun' },
            { title: 'Para Kazan', modules: monetizationModules, id: 'monetization' },
            { title: 'Web3', modules: web3Modules, id: 'web3' },
            { title: 'Güvenlik', modules: securityModules, id: 'security' }
          ].map((section) => (
                  ...
                  {section.modules.map((module) => (
                    <div key={module.id} className={`group relative bg-[#181920] border border-white/5 rounded-xl p-5 hover:border-primary/30 transition-all duration-200 hover:-translate-y-0.5 ${module.isLocked ? 'opacity-70' : ''}`}>
                      ...
                      <button 
                          ...
                          className={`w-full py-2 rounded-lg text-sm font-medium border transition-colors flex items-center justify-center gap-2 ${
                          module.isEnabled 
